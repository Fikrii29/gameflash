// =====================================================
// GAMEFLASH - Digiflazz API Integration
// =====================================================
...
const DigiFlazz = {
  ...
  getCredentials() {
    ...
    return {
      ...
      apiKey: settings.digiflazzApiKey || '',
      production: settings.digiflazzProduction || false
    };
  },
  ...
  async md5(message) {
    ...
    // Note: Digiflazz uses MD5, but browser doesn't have native MD5
    // We use a simple MD5 implementation here
  ...
  simpleMD5(str) {
    ...
    const hex = [a, b, c, d];
    return hex.map(n => {
      const h = (n + (n < 0 ? 0x100000000 : 0)).toString(16).padStart(8, '0');
  ...
  async getPriceList() {
    ...
    if (!username || !apiKey) {
      return { success: false, error: 'Digiflazz credentials not configured' };
    }
    const sign = this.simpleMD5(username + apiKey + 'pricelist');
    try {
      const response = await fetch(`${this.BASE_URL}/price-list`, {
        method: 'POST',
  ...
  // Create transaction
  async createTransaction({ refId, username: buyerUsername, sku, customerNo }) {
    const { username, apiKey } = this.getCredentials();
    if (!username || !apiKey) {
      return { success: false, error: 'Digiflazz credentials not configured' };
    }
    const sign = this.simpleMD5(username + apiKey + refId);
    try {
      const response = await fetch(`${this.BASE_URL}/transaction`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, ref_id: refId, buyer_sku_code: sku, customer_no: customerNo, sign })
      });
      const data = await response.json();
      return { success: data.data?.rc === '00', data: data.data, raw: data };
    } catch (err) {
      return { success: false, error: err.message };
    }
  },
  ...
  async checkBalance() {
    const { username, apiKey } = this.getCredentials();
    if (!username || !apiKey) return { success: false, error: 'Not configured' };
    const sign = this.simpleMD5(username + apiKey + 'depo');
    try {
      const response = await fetch(`${this.BASE_URL}/cek-saldo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cmd: 'deposit', username, sign })
      });
      const data = await response.json();
      return { success: true, balance: data.data?.deposit };
    } catch (err) {
      return { success: false, error: err.message };
    }
  }
