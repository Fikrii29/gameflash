...
function showPaymentModal(paymentResult) {
  ...
  if (selectedPayment.category === 'qris') {
    // Tokopay returns qr_link (image URL) or qr_string (raw QRIS string)
    const qrImageUrl = paymentResult.qrCode || paymentResult.data?.qr_link || (paymentResult.data?.qr_string && paymentResult.data.qr_string.startsWith('http') ? paymentResult.data.qr_string : null) || '';
    const qrContent = qrImageUrl && qrImageUrl.startsWith('http')
      ? `<img src="${qrImageUrl}" style="width:180px;height:180px;object-fit:contain;" onerror="this.outerHTML='<div style=\'text-align:center;padding:16px;font-size:0.75rem;color:#666;\'>QR tidak tersedia</div>'" />`
      : qrImageUrl
        ? `<div style="font-size:0.5rem;word-break:break-all;padding:8px;color:#333;">${qrImageUrl.substring(0, 100)}...</div>`
        : `<div style="text-align:center;padding:16px;"><div style="font-size:2rem;">📱</div><div style="font-size:0.75rem;color:#666;margin-top:4px;">${currentTransaction.invoiceId}</div></div>`;
    paymentContent = `
      <div class="qr-box">
