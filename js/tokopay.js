...
const TokoPay = (() => {
  ...
  function simulatePayment(refId, method, amount) {
    ...
    if (m.category === 'qris') {
      // QRIS string simulasi menggunakan format QR yang valid agar bisa di-scan
      // Format: URL payment page (bisa di-scan sebagai QR code biasa)
      const qrString = 'https://qris.id/qr/?ref=' + refId + '&amount=' + amount + '&merchant=GAMEFLASH';
      const qrImageUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=10&data=' + encodeURIComponent(qrString);
